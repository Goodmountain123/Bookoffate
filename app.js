...
async function captureBook() {
  ...
  try {
    canvas = await window.html2canvas(book, {
      backgroundColor: '#ead4a8',                // 양피지와 같은 색 — 누락 부분 자연 융화
      scale: 2,                                  // 2배. 3배는 iOS에서 큰 캔버스 한계로 흐려질 수 있음
      useCORS: true,
      allowTaint: false,
      logging: false,
      imageTimeout: 0,
      ...
      onclone: (clonedDoc) => {
        // === 캡쳐용 강제 CSS 주입 ===
        // (스타일시트 캐시되거나 .capturing 클래스가 안 잡혀도 무조건 적용됨)
        ...
        // === 클론 .book 인라인 스타일 강제 ===
        // 그라데이션·인셋 그림자가 망가지는 걸 막기 위해 단색·단순 그림자로 덮어씀
