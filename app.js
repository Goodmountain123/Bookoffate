...
// SAVE / SHARE
// ============================================
// html2canvas-pro: 원본 html2canvas의 유지보수 포크. 최신 CSS·렌더링 안정성 더 좋음.
...
function loadHtml2Canvas() {
  ...
  return new Promise((resolve, reject) => {
    ...
    script.onerror = () => reject(new Error('html2canvas-pro 로드 실패'));
    document.head.appendChild(script);
  });
}

// 캡쳐 전 사용 폰트 명시적 로딩 — display:swap 으로 인한 글꼴 미로드 캡쳐 방지
      ...
      onclone: (clonedDoc) => {
        ...
        style.textContent = `
          * {
          ...
          /* 의사 요소(텍스쳐/비네팅) 완전 제거 — html2canvas가 잘못 렌더링하는 주범 */
          .book::before,
          .book::after {
          ...
          .capture-footer { display: block !important; }
        `;
        clonedDoc.head.appendChild(style);

        // === 클론 .book 인라인 스타일 강제 ===
        const clonedBook = clonedDoc.querySelector('.book');
        if (clonedBook) {
          clonedBook.style.setProperty('background', '#ead4a8', 'important');
          clonedBook.style.setProperty('background-image', 'none', 'important');
          clonedBook.style.setProperty('box-shadow',
            '0 0 0 6px #4a1316, 0 0 0 7px #8a6b3a', 'important');
        }
      },
    });
  } finally {
