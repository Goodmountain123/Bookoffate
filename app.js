...
function renderResult(data) {
  ...
  // 답안지 렌더링 (선택지 4개 + ● 선택 표시)
  const responsaEl = document.getElementById('rResponsa');
  responsaEl.innerHTML = '';
  responses.forEach((r, i) => {
    const block = document.createElement('div');
    block.className = 'responsa-item';

    const q = document.createElement('div');
    q.className = 'responsa-question';
    q.textContent = `Q${i + 1}. ${r.question}`;
    block.appendChild(q);

    const choices = document.createElement('ul');
    choices.className = 'responsa-choices';
    (r.choices || []).forEach(c => {
      const li = document.createElement('li');
      const isSelected = c === r.selected;
      li.className = isSelected ? 'responsa-choice selected' : 'responsa-choice';
      li.textContent = c;
      choices.appendChild(li);
    });
    block.appendChild(choices);

    responsaEl.appendChild(block);
  });
