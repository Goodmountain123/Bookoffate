...
// SAVE / SHARE
// ============================================
// modern-screenshot: SVG foreignObject 기반. 브라우저가 직접 렌더링한 결과를
// 그대로 변환하므로 폰트·그라데이션·shadow 모두 깔끔하게 나옴.
const SCREENSHOT_URL = 'https://cdn.jsdelivr.net/npm/modern-screenshot@4.6.0/dist/index.umd.js';
...
function loadScreenshotLib() {
  ...
  return new Promise((resolve, reject) => {
    ...
    script.onerror = () => reject(new Error('modern-screenshot 로드 실패'));
...
async function captureBookAsBlob() {
  ...
  try {
    blob = await window.modernScreenshot.domToBlob(book, {
      ...
      type: 'image/png',
      style: {
        // 클론 노드에 직접 인라인으로 들어가는 스타일 (가장 강력함)
      },
      // 의사 요소 처리는 모던 스크린샷이 내부적으로 잘 처리하지만
      // 안전을 위해 필터로 일부 제외 가능
    });
  } finally {
